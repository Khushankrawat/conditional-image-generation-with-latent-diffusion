...
diffusion_model_path = 'models/cldm_cldm_32/unet/37989' # Your path here
...
def generate_images(job_id, prompt, inf_steps, n_images, stream_freq=50, latent_input_base64=None, start_timestep=0, model_path=diffusion_model_path, save_dir=None):
    ...
    def do_collage(images, num_rows, num_cols):
        ...
        if not images:
            print("No images provided for collage")
            return None
    ...
    def collage_images():
        ...
        # Skip collage creation if no images found
        if num_images == 0:
            print("No images found for collage")
            return
        ...
        # Only create collage if we have images
        if images:
            collage_image = do_collage(images, num_rows, num_cols)
            if collage_image:  # Check if collage was created successfully
    ...
    def collage_final_images():
        ...
        # Skip collage creation if no images found
        if num_images == 0:
            print("No final images found for collage")
            return
