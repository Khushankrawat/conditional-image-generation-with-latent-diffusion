...
data_dir = "./archive/archive/img_align_celeba/img_align_celeba" # Your dataset location
label_file = "./archive/text_labels.json" # Your dataset location
...
class CelebADataset(Dataset):
    def __init__(self, root, transform=None, target_transform=None, center_mean=True, text_labels=None, return_filename_as_target=False):
        ...
        self.samples.sort()  # Sort for consistent ordering
    ...
    def __getitem__(self, index):
        ...
        target = 0  # Default target for compatibility
        if self.text_labels:
            text_labels = self.text_labels.get(file_id, [""])
            text_labels = [t for t in text_labels if len(t.split()) <= 7]
            target = random.choice(text_labels) if text_labels else ""
...
def load_data(img_size, batch_size, use_albumenations, sample=None, center_mean=True, text_labels=False, train_ratio=1.0, test_size=None, get_filepaths = False):
    ...
    train_dataset = CelebADataset(root=data_dir, transform=transform, center_mean=center_mean, text_labels=text_labels, return_filename_as_target=get_filepaths)
